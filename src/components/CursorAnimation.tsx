...
const CursorAnimation: React.FC<RibbonsProps> = ({
  colors = ['#06b6d4'],
  baseSpring = 0.03,
  baseFriction = 0.9,
  baseThickness = 8,
    ...
    const vertex = `
      ...
      vec4 getPosition() {
          ...
          if(uEnableShaderEffect > 0.5) {
            float wave = sin(uTime * 2.0 + current.x * 15.0) * sin(uTime * 1.5 + current.y * 10.0);
            current.xy += normal * wave * uEffectAmplitude;
          }
          return current;
      }
  ...
  return <div ref={containerRef} className="fixed inset-0 pointer-events-none z-[1000]" />;
};
